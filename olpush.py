...
def main ():
	...
	for reportFilename in os.listdir (reportDirectory):
		...
		if reportFilenameWithoutExt == "index":
			continue
	...
	# upload the index now
	# wow such copy pasta

	indexSource = open("%s/%s" % (reportDirectory, "index.html")).read()
	indexSource = indexSource.replace("\n", "")

	content_url = 'https://www.openlearning.com/courses/99luftballons/Cohorts/ClassOf2014/Groups/Tutors/MagicalMarkingPages'
